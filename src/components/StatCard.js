    ...
    >
      ...
      <div className="relative">
        <div className="flex items-start justify-between gap-3">
          <p className={`min-w-0 uppercase font-semibold text-slate-600 ${isCompact ? "text-[10px] tracking-[0.1em]" : "text-[11px] tracking-[0.12em]"}`}>
            {label}
          </p>
          <div className={`shrink-0 ${textColorMap[color]} ${isCompact ? "text-lg md:text-xl" : "text-xl md:text-2xl"}`}>
            {icon}
          </div>
        </div>

        <p
          className={`${textColorMap[color]} mt-2 font-bold leading-tight tabular-nums [text-wrap:balance] ${
            isCompact ? "text-lg md:text-xl" : "text-[clamp(1.25rem,2.2vw,2rem)]"
          }`}
        >
          {value}
        </p>
        ...
        {trend && (
          <p className={`mt-2 text-[11px] font-semibold ${trendPositive ? "text-emerald-600" : "text-rose-600"}`}>
            {trendPositive ? "↑" : "↓"} {trend}
          </p>
        )}
      </div>
